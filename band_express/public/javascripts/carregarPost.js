...
// let newPost = async () => {
//     let lastDiv = document.querySelector("#publicacao > div:last-child");
...
//         // requisição de mais post

//         carregarPost();


//         // var newPost = document.createElement("div");
//         // newPost.innerHTML = "Novo Post 🧡";
//         // publicacao.appendChild(newPost);
...
async function maisPosts() {
    try {
        ...
        let buscarPost = await fetch(`/home/carregarPost/${page}`);
    ...
    for (const item of list) {
        ...
        divCard.className = 'card-feed';
            ...
            divIcon.id = 'grupoIconPost';
                ...
                let buttonCurtir = document.createElement('button');
                buttonCurtir.type = "button";

                    let iconeCurtir = document.createElement('i');
                    iconeCurtir.id = `iconeCurtir${item.id_post}`;
                    iconeCurtir.className = 'fas fa-star';
                    curtidasJs.forEach(curtida => {
                        if (item.id_post == curtida.id_post) {
                            iconeCurtir.className = 'fas fa-star curtiu';
                        }
                    });
                    buttonCurtir.appendChild(iconeCurtir);
                
                divIcon.appendChild(buttonCurtir);
