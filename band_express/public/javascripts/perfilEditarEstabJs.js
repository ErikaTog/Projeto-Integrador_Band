...
const load = async () => {
    ...
    const response = await fetch('/perfil/editar/estabelecimento/dados', options);
    const dadosBack = await response.json();

    for (let i = 0 ; i < dadosBack.dados.length ; i++) {
        diasAdicionados.push({
            dia: dadosBack.dados[i].dia,
            abertura: dadosBack.dados[i].horario_abertura, 
            fechamento: dadosBack.dados[i].horario_fechamento 
        });
        for(let valor of diasDaSemana){
            valor.dia == dadosBack.dados[i].dia ? valor.on = 0 : null;
        }
    }
