...
async function buscarVideo() {
    try {
        ...
        let musicasBuscadas = await fetch(`/perfil/banda/carregarVideo/${id_usuario}/${page}`);
    ...
    for (const item of list) {
        ...
        divVideo.className = 'musica-item col-xl-6 col-sm-12';
            ...
            divLixo.className = 'icon-border-none';
                ...
                aLixo.href = `/perfil/editar/banda/deletarVideo/${item.id_video}`
...
async function buscarAudio() {
    try {
        let page = buttonCarregar.value;
        let musicasBuscadas = await fetch(`/perfil/banda/carregarAudio/${id_usuario}/${page}`);
        let listaMusicas = await musicasBuscadas.json();
    ...
    for (const item of list) {
        ...
        let divLixo = document.createElement('div');
            divLixo.className = 'icon-border-none';
                ...
                let aLixo = document.createElement('a');
                aLixo.href = `/perfil/editar/banda/deletarAudio/${item.id_audio}`;
